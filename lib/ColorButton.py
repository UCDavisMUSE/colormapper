...
class ColorButton(wx.lib.buttons.GenButton):
    def __init__(self, parent, color = (0, 0, 0), size = (30, 30)):
        wx.lib.buttons.GenButton.__init__(self, parent, -1, size = size)
        self.SetBackgroundColour(color)
        self.Bind(wx.EVT_BUTTON, self.OnButtonPress, self)

    def OnButtonPress(self, event):
        # Set the initial color of the color dialog box to the current button color
        data = wx.ColourData()
        data.SetChooseFull(True)
        currentColor = self.GetBackgroundColour()
        data.SetColour(wx.Colour(currentColor[0], currentColor[1], currentColor[2]))
